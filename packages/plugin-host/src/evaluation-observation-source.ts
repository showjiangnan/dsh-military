...
export class DshEvaluationObservationSource implements EvaluationDataSource {
  ...
  async collect(
    ...
    const dispatchReceipts = new SqliteStateRecords(
      this.#database,
      this.#tenantId,
    ).listSync<ArtifactDispatchPolicyReceipt>(
    ...
    for (const binding of this.#bindings()) {
      ...
      for (const lease of leases) {
        ...
        const pricing = exactDispatchPricing(
          dispatchReceipts,
          binding,
          metrics.modelUsage,
        )
        ...
        const record: EvaluationAttemptRecord = {
          ...
          usage: {
            ...
            ...(pricing.estimatedCostUsd === undefined
              ? {}
  ...
  async #parentWakeup(
    ...
    to: number,
    signal: AbortSignal,
  ): Promise<{ readonly observed: boolean; readonly woke: boolean }> {
    const parent = await this.#session(rootSessionId, signal)
    return {
      observed: parent !== null,
      woke: parent?.events.some(event =>
        event.time >= from
        && event.time <= to
...
function attemptOutcome(
  ...
  parentWakeup: { readonly observed: boolean; readonly woke: boolean },
  ...
  return {
    ...
    parentWakeup: !completed || parentWakeup.woke,
    ...
    authority: {
      taskSettlementObserved: events.some(value =>
        value.type === 'task/accepted' || value.type === 'task/cancelled'),
      missionSettlementObserved: events.some(value =>
        value.type === 'mission/completed' || value.type === 'mission/cancelled'),
      specsOutcomeObserved: events.some(value =>
        value.type === 'specs/commit-recorded')
        || binding.departmentId !== 'engineer-corps',
      integrationOutcomeObserved: acceptedEvent?.payload.integrationRequired === false
        || integrated
        || regressionEscape,
      parentWakeupObserved: !completed || parentWakeup.observed,
    },
  }
}
...
function inspectSession(
  ...
  const modelUsage: Array<SessionMetrics['modelUsage'][number]> = []
...
function exactDispatchPricing(
  receipts: readonly ArtifactDispatchPolicyReceipt[],
  binding: AgentExecutionBinding,
  usage: SessionMetrics['modelUsage'],
): Pick<
  EvaluationAttemptRecord['usage'],
  'costStatus' | 'pricingVersion' | 'estimatedCostUsd'
> {
  if (usage.length === 0) {
    return { costStatus: 'PROVIDER_PRICING_UNAVAILABLE' }
  }
  ...
  for (const item of usage) {
    const receipt = receipts.find(candidate =>
      ...
      && candidate.dispatch.agentId === String(binding.agent.agentId)
      && candidate.dispatch.agentGeneration === binding.agent.generation
      && candidate.dispatch.turn === item.turn
      && candidate.dispatch.step === item.step
      && candidate.provider === item.provider
      && candidate.model === item.model)
    const pricing = receipt?.pricingSnapshot
    if (
      pricing === undefined
    ...
    estimatedCostUsd += (
      item.inputTokens * pricing.inputPerMillionTokens
      + item.outputTokens * pricing.outputPerMillionTokens
    ) / 1_000_000
    versions.push(`${item.provider}/${item.model}@${pricing.version}`)
  }
  const exactVersions = [...new Set(versions)].sort()
  return {
    costStatus: 'ESTIMATED',
    pricingVersion: exactVersions.length === 1
      ? exactVersions[0]!
      : `price-snapshot-set:${sha256(stableJson(exactVersions))}`,

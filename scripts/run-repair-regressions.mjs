...
assert.match(sqliteLedger, /transactionAsync\(/u)
...
assert.match(settingsCenter, /data-role-prompt-editor/u)
assert.match(settingsCenter, /恢复全部角色自带提示词/u)
assert.match(settingsCenter, /Military-Specs 工作区/u)
assert.match(web, /dsh-military: settings centre overlay/u)
assert.match(roleWorkbench, /militaryControlPlane\/snapshot/u)
assert.match(roleWorkbench, /militaryControlPlane\/execute/u)
assert.match(roleWorkbench, /恢复自带提示词/u)
assert.match(controlPlaneRemote, /case 'RESTORE_ROLE_PROMPT'/u)
assert.match(operationsCenter, /militaryOperations\/snapshot/u)
assert.match(operationsCenter, /militaryOperations\/execute/u)
assert.match(workspaceCenter, /militaryWorkspace\/snapshot/u)
assert.match(workspaceCenter, /INSPECT_WORKSPACE/u)
assert.match(benchmarkCenter, /militaryBenchmark\/snapshot/u)
...
assert.match(pipeline, /if \(admitted\) \{[\s\S]*settleToolExecutionBudget/u)
...
assert.match(hostRuntime, /application\.runtime\.cancelTask/u)
